...
export default function AddCarModal({ isOpen, onClose, onCarAdded }) {
  const [formData, setFormData] = useState({ brand: '', model: '', daily_rate: '', image_url: '', description: '' })
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      onCarAdded()
      setFormData({ brand: '', model: '', daily_rate: '', image_url: '', description: '' })
      setImageFile(null)
      onClose()
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
      <div className="bg-[#1c1c1e] border border-white/10 w-full max-w-md rounded-3xl p-8 shadow-2xl relative overflow-hidden">
        ...
        {error && <div className="bg-red-500/10 border border-red-500/50 text-red-400 p-3 rounded-lg mb-4 text-sm relative z-10">{error}</div>}
