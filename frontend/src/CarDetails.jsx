...
export default function CarDetails({ cars, session }) {
  ...
  useEffect(() => {
    fetch('https://sura-rentals-api.onrender.com/api/reviews/')
      .then(res => res.json())
      .then(data => { if(Array.isArray(data)) setReviews(data.filter(r => r.car.toString() === id)) })
      .catch(err => console.error(err))

    if (session) {
      fetch('https://sura-rentals-api.onrender.com/api/wishlists/')
        .then(res => res.json())
        .then(data => {
  ...
  const toggleLike = async () => {
    ...
    try {
      if (!isLiked) {
        const res = await fetch('https://sura-rentals-api.onrender.com/api/wishlists/', {
          method: 'POST',
        ...
        await fetch(`https://sura-rentals-api.onrender.com/api/wishlists/${wishlistId}/`, { method: 'DELETE' })
        setWishlistId(null)
      }
  ...
  const handleReviewSubmit = async (e) => {
    ...
    const res = await fetch('https://sura-rentals-api.onrender.com/api/reviews/', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  const handleDeleteReview = async (reviewId) => {
    try {
      const res = await fetch(`https://sura-rentals-api.onrender.com/api/reviews/${reviewId}/`, { method: 'DELETE' })
      if (res.ok) {
